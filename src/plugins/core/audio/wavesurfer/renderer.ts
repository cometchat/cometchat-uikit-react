...
class Renderer extends EventEmitter<RendererEvents> {
  ...
  private initEvents() {
    ...
    // Add a click listener (stored for cleanup in destroy())
    ...
    // Add a double click listener (stored for cleanup in destroy())
    this.dblclickHandler = (e: MouseEvent) => {
      const [x, y] = getClickPosition(e);
      this.emit('dblclick', x, y);
    };
    this.wrapper.addEventListener('dblclick', this.dblclickHandler);
    ...
    // Add a scroll listener (stored for cleanup in destroy())
    this.scrollHandler = () => {
  ...
  destroy() {
    ...
    if (this.dblclickHandler) {
      this.wrapper.removeEventListener('dblclick', this.dblclickHandler);
      this.dblclickHandler = undefined;
    }
