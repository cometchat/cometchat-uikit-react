...
import type { CometChatAIAssistantChatHistoryProps } from './ai.types';
...
function formatDate(timestamp: number, getLocalizedString: (key: string) => string): string {
  ...
  if (msgDate.getTime() === today.getTime()) return getLocalizedString('date_today');
  if (msgDate.getTime() === yesterday.getTime()) return getLocalizedString('date_yesterday');
...
export const CometChatAIAssistantChatHistory: React.FC<CometChatAIAssistantChatHistoryProps> = ({
  user,
  group,
  hideNewChat = false,
  ...
  const fetchMessages = useCallback(
    ...
    [onMessageClick, onEmpty, onError]
  );

  // Initialize on mount / user/group change
  useEffect(() => {
    ...
    <div className={rootClasses}>
      ...
      <div className={'cometchat-ai-assistant-chat-history__header'}>
        ...
        <button
          ...
          onClick={onClose}
        >
          <span className={'cometchat-ai-assistant-chat-history__close-icon'} aria-hidden="true" />
        </button>
      </div>
                ...
                {showSeparator && (
                  ...
                  >
                    <span className={'cometchat-ai-assistant-chat-history__date-separator-text'}>
                      {formatDate(message.getSentAt(), getLocalizedString)}
                    </span>
                  </li>
                )}
                <li
