...
function convertFormattingHtmlToMarkdown(text: string): string {
  ...
  // Protect code spans before conversion so their content stays literal and is never
  // DOM-parsed (which could mis-format it or execute embedded HTML via innerHTML).
  const codePlaceholders: string[] = [];
  let decoded = text.replace(/```[\s\S]*?```|`[^`]*`/g, match => {
    const idx = codePlaceholders.length;
    codePlaceholders.push(match);
    return `\u200B__CODE_${String(idx)}__\u200B`;
  });
  ...
  // SECURITY: convertHtmlToMarkdown parses via innerHTML, so sanitize first — an
  // <img onerror>/<svg onload> would otherwise fire during parsing.
  ...
  // Restore code spans; the markdown formatter downstream escapes + wraps their content.
  const withCode = result.replace(/\u200B__CODE_(\d+)__\u200B/g, (_, idx) => {
    return codePlaceholders[parseInt(idx as string, 10)] ?? '';
  });
...
export const CometChatTextBubble: React.FC<CometChatTextBubbleProps> = ({
  ...
  // Self-extract: render an explicit `text` (e.g. a media caption) when provided,
  // otherwise pull the content from the message itself.
  const effectiveText = useMemo(() => {
    if (typeof text === 'string') return text;
    try {
      return (message as CometChat.TextMessage | undefined)?.getText() ?? '';
    } catch {
      return '';
    }
  }, [text, message]);

  // Configure the mentions formatter from the message (logged-in user, alignment,
  // mentioned users) so mention styling resolves correctly. Previously done by the plugin.
  useMemo(() => {
    for (const formatter of textFormatters) {
      if (formatter instanceof CometChatMentionsFormatter) {
        ...
        formatter.setMessageBubbleAlignment(isSentByMe ? 'right' : 'left');
        try {
          const mentioned =
            (message as CometChat.TextMessage | undefined)?.getMentionedUsers() ?? [];
          if (mentioned.length > 0) formatter.setUsers(mentioned);
        } catch {
          /* message has no mentions accessor */
        }
      }
    }
  }, [textFormatters, loggedInUser, isSentByMe, message]);
  ...
  // The text already contains SDK mention tokens (<@uid:xxx>) and markdown formatting.
  // applyFormatters handles HTML→markdown conversion only when the text itself contains
  // HTML tags (from other platforms/older clients).
  const formattedHtml = useMemo(() => {
    return applyFormatters(effectiveText, textFormatters, message);
  }, [effectiveText, textFormatters, message]);
  ...
  const translatedText = useMemo(
    () => extractTranslation(message as CometChat.TextMessage | undefined),
    [message]
  );
  ...
  const mentionedUsers: CometChat.User[] = useMemo(() => {
    try {
      return (message as CometChat.TextMessage | undefined)?.getMentionedUsers() ?? [];
    } catch {
      return [];
    }
  }, [message]);
  ...
  const handleTextClick = useCallback(
    ...
    [publish, mentionedUsers, getCurrentWindow]
  );

  // Handle link preview click
  const handleLinkPreviewClick = useCallback(
    (url: string) => {
      if (url) {
        getCurrentWindow().open(url, '_blank', 'noopener,noreferrer');
      }
