...
import { useCometChatMessageComposerContext } from '../CometChatMessageComposer/CometChatMessageComposer.context';
...
const CometChatAIAssistantChatComponent: React.FC<CometChatAIAssistantChatProps> = ({
  ...
  const handleMessageSent = useCallback(
    (message: CometChat.BaseMessage) => {
      ...
      // Add message to history list (matches v6 behavior)
      if (
        ...
        historyRef.current?.addMessage(message);
      }

      onSendButtonClick?.(message);
    },
    [chatId, loggedInUser, onSendButtonClick]
  );
