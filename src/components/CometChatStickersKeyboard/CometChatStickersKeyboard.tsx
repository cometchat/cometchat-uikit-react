/* eslint-disable @typescript-eslint/no-unnecessary-condition, @typescript-eslint/prefer-nullish-coalescing, @typescript-eslint/no-floating-promises, @typescript-eslint/no-misused-promises */
/**
...
export const CometChatStickersKeyboard: React.FC<CometChatStickersKeyboardProps> = ({
  ...
  useEffect(() => {
    ...
    getCurrentDocument().addEventListener('keydown', handleKey);
    return () => {
      getCurrentDocument().removeEventListener('keydown', handleKey);
    };
  ...
  useEffect(() => {
    if (state === 'loaded' && autoFocus) {
      const timer = setTimeout(() => {
        const firstTab = tabsRef.current?.querySelector('[role="tab"]') as HTMLElement | null;
        firstTab?.focus();
      }, 100);
      return () => {
        clearTimeout(timer);
  ...
  const handleStickerKeyDown = useCallback(
    (e: React.KeyboardEvent, sticker: CometChatStickerItem, index: number) => {
      ...
      setTimeout(() => {
        const items = getCurrentDocument().querySelectorAll('[role="gridcell"]');
        (items[newIndex] as HTMLElement)?.focus();
      }, 0);
    },
    [currentStickers.length, getCurrentDocument, handleStickerClick]
  );
