...
class WaveSurfer extends Player<WaveSurferEvents> {
  ...
  private initPlayerEvents() {
    ...
    this.mediaSubscriptions.push(
      ...
      this.onMediaEvent('error', err => {
        this.emit('error', (err as any).error);
      })
    );
  }
  ...
  private async loadAudio(
    ...
    if (!blob && !channelData) {
      ...
      const win = this.options.iframeWindow || window;
      if (
        (win as Window & { AbortController?: typeof AbortController }).AbortController &&
        !fetchParams.signal
      ) {
   ...
   * @param quality The quality of the exported image
   * @param type The type of the exported image
   * @returns A promise that resolves with an array of data-URLs or blobs
   */
