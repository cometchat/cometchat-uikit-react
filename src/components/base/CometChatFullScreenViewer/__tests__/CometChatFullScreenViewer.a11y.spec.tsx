...
describe('CometChatFullScreenViewer a11y', () => {
  ...
  it('passes axe-core audit with zero violations (video mode)', async () => {
    const { container } = renderViewer({
  ...
  it('aria-label changes based on media type', () => {
    ...
    expect(screen.getByRole('dialog')).toHaveAttribute('aria-label', 'Full screen video');

    rerender(<CometChatFullScreenViewer.Root onClose={vi.fn()} url="x" mediaType="audio" />);
    expect(screen.getByRole('dialog')).toHaveAttribute('aria-label', 'Full screen audio');
