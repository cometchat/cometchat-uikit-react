...
/**
 * Raw emoji dataset. Each entry is `{ [categoryKey]: { id, name, symbol, emojis } }`.
 * Transformed into `CometChatEmojiKeyboardCategoryData[]` via `getDefaultEmojiCategories()`.
 */
const Emojis = [
  {
    people: {
...
let cachedCategories: CometChatEmojiKeyboardCategoryData[] | null = null;

/**
 * Returns the built-in emoji categories, lazily transformed from the raw dataset.
 * The result is cached after the first call.
 */
export function getDefaultEmojiCategories(): CometChatEmojiKeyboardCategoryData[] {
  if (cachedCategories) return cachedCategories;

  cachedCategories = Emojis.map(entry => {
    const vals = Object.values(entry)[0] as {
      id: string;
      name: string;
      symbol: string;
      emojis: Record<string, { char: string; keywords: string[] }>;
    };
    return {
      id: vals.id,
      name: vals.name,
